...
// ── ۴.۵) تعداد فقط نیم‌جین (۶) یا جینِ کامل (۱۲) ───────────────────────────
// خواستِ مالک: «ما فقط ۶ عددی و ۱۲ تایی داریم و بهش ارور هم بده».
// گفتنِ قاعده بدونِ اجرا کردنش بدتر از نگفتنش است: مشتری ۳ تا سفارش می‌دهد،
// سفارش ثبت می‌شود، و مالک باید زنگ بزند و درستش کند. پس هر سه را می‌سنجیم:
// عددِ غلط رد شود، پیامِ ارور بگوید چرا، و هر دو عددِ درست قبول شوند.
console.log("\n━━━ بسته‌بندیِ فروش ━━━");
for (const bad of ["۳", "۱", "۷", "۱۳", "۲۴"]) {
  ...
  check(`ارورِ ${bad} می‌گوید فقط ۶ و ۱۲`, /۶ عدد/.test(t) && /۱۲ عدد/.test(t), true);
}
for (const ok of ["۶", "۱۲"]) {
  reset({ step: "qty", cart: [], temp_product_id: "p1" });
  await send(msg(ok));
