...
def bot_checks(src, code):
    ...
    head("۷) هم‌خوانیِ کپیِ مرجع (OPS-06، OPS-08)")

    # OPS-08 — استقرار **ادغام نمی‌کند**. یک بار نسخه‌ی ۴۲ از شاخه‌ای مستقر شد که
    # ۴۹ کامیت از main عقب بود و کارِ یک نشستِ دیگر را بی‌صدا از رباتِ زنده پاک
    # کرد. جلوگیری‌اش یک گامِ گیتی است که در bot/README.md نوشته شده — و چون
    # «هشدارِ متنی بعد از چند بار خوانده نمی‌شود»، اینجا وجودش سنجیده می‌شود تا
    # بی‌سروصدا حذف نشود. این بررسی *اجرای* گام را ثابت نمی‌کند، فقط ماندنش را.
    rd = open(BOT_README, encoding="utf-8").read()
    if "merge-base --is-ancestor" not in rd:
        bad("روالِ استقرارِ ربات گامِ «پایه‌ام به‌روز است؟» را ندارد (OPS-08) — "
            "بدونش استقرار از شاخه‌ی عقب‌مانده کارِ دیگران را پاک می‌کند. "
            "`git merge-base --is-ancestor origin/main HEAD` را در bot/README.md برگردان")
    elif not re.search(r"ادغام نمی‌کند", rd):
        bad("bot/README.md گامِ گیتی را دارد ولی نمی‌گوید **چرا** — "
            "«استقرار ادغام نمی‌کند» باید صریح نوشته شود، وگرنه نفرِ بعد دورش می‌زند")
    else:
        ok("روالِ استقرار گامِ به‌روز بودنِ پایه را دارد و دلیلش را هم می‌گوید (OPS-08)")
