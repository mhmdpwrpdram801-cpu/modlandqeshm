...
1. «اجرا می‌شود» یا «تست سبز است»: فرمان دقیق + خروجی واقعی آن
   (تعداد تست، خطاها، exit code)؛ خلاصه‌کردن به‌جای نقل خروجی کافی نیست.
2. ظاهر و چیدمان: اسکرین‌شات واقعاً گرفته و دیده‌شده، دست‌کم در یک عرض موبایل
   و یک عرض دسکتاپ؛ «رندر شد و خطا نداد» شواهد نیست.
3. کنترل‌ها (دکمه، لینک، تب، مودال، منو): کلیک/لمس واقعی روی همان عنصر و مشاهده نتیجه؛
   صدا زدن تابع پشت آن مسیر واقعی کاربر را نمی‌آزماید.
4. عدد، مبلغ، تخفیف، موجودی: مقایسه با مقدار دستی محاسبه‌شده، شامل حداقل یک مقدار
   مرزی (صفر، منفی، سقف، تخفیف مساوی جمع).
5. تم و رنگ: اندازه‌گیری رنگ پس‌زمینه، متن، border و focus روی همان عناصر در هر دو تم؛
   باز شدن صفحه در تم تیره شواهد نیست.
6. «رفع شد»: اجرای دوباره دقیقاً همان بررسی‌ای که قبلاً شکست خورده بود و نتیجه جدید.
7. نسخه جدید PWA: نصب نسخه قدیمی، انتشار نسخه جدید، مشاهده دریافت آن روی همان دستگاه.
8. نبودِ ابزار، دستگاه یا دسترسی: صریح بنویس «بررسی نشد» و علت؛
   «احتمالاً درست است» تحویل نده.

فرمان یا اسکریپت هر بررسی را داخل پروژه نگه دار، نه فقط در متن پاسخ.
</evidence_rules>
...
- برای بارگذاری تنبل کتابخانه‌های موردی به performance_and_quality نگاه کن.
</javascript_requirements>
...
- کتابخانه‌ای که فقط در بخشی از اپ لازم است (خروجی PDF/Excel، چارت، ادیتور)
  با dynamic import در لحظه نیاز بار شود، نه در شروع.
...
| `<javascript_requirements>` | یک خط ارجاع به `performance_and_quality` برای بارگذاری تنبل؛ خود قاعده فقط یک‌جا نگه داشته شد | — |
