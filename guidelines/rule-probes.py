...
شش ماهِ دیگر کسی دوباره همین راه را نرود:
  ...
  · **`STACK-05` — نیمه‌ی `any`ِ آن.** روی همین مخزن شمرده شد: **۵۱ مورد**،
    تقریباً همه در `bot/index.ts` که یک تابعِ لبه‌ی Deno بدونِ `tsconfig` است و
    ...
    ✅ **نیمه‌ی دیگرش اضافه شد** (پایین‌تر، `probe_ts_suppress`): سرکوبِ کورِ
    تایپ دوتایی و قطعی است و در کلِ مخزن **۱ مورد** داشت، که اصلاح شد و ربات
    با آن مستقر شد. یعنی یک قاعده لازم نیست تمام‌وکمال سنجیدنی باشد تا
    نیمه‌ی سنجیدنی‌اش دروازه بگیرد — فقط باید صریح بگویی کدام نیمه.
...
# ───────────────────────────── STACK-05: سرکوبِ کورِ تایپ ممنوع
#
# **فقط نیمه‌ی سنجیدنیِ این قاعده.** نیمه‌ی `any` عمداً اینجا نیست (بالا با عدد
# توضیح داده شده). ولی فرقِ `@ts-ignore` و `@ts-expect-error` دوتایی و قطعی است:
# اولی هر خطایی را برای همیشه می‌بلعد، دومی اگر روزی خطا **نبود** خودش قرمز
# می‌دهد — یعنی سرکوب هم مستند می‌شود هم تاریخِ انقضا دارد.
#
# روی همین مخزن اندازه گرفته شد: **دقیقاً ۱ مورد** (در `bot/index.ts` برای
# `duplex: 'half'`)، که همان‌جا اصلاح شد و ربات با آن مستقر شد. نرخِ هشدارِ
# نادرست صفر است، پس معیارِ ورودِ این فایل را دارد.
TS_EXT = {".ts", ".tsx", ".mts", ".cts"}


def probe_ts_suppress(root: str) -> None:
    needle = "@ts-" + "ignore"          # تا خودِ این فایل الگو را نداشته باشد
    files, hits = 0, []
    for path in walk(root):
        if os.path.splitext(path)[1].lower() not in TS_EXT:
            continue
        rel = os.path.relpath(path, root)
        if rel.startswith(OWN_FIXTURES + os.sep):
            continue
        files += 1
        try:
            text = open(path, encoding="utf-8", errors="ignore").read()
        ...
        for i, line in enumerate(text.splitlines(), 1):
            if needle in line:
                hits.append(f"{rel}:{i}")
    if not files:
        skip("STACK-05", "فایلِ TypeScript نیست — چیزی برای سنجیدن وجود ندارد")
    elif hits:
        bad("STACK-05", f"سرکوبِ کورِ تایپ در {len(hits)} جا: " + " · ".join(hits[:4]) +
            ("" if len(hits) <= 4 else f" (و {len(hits)-4} مورد دیگر)") +
            " — به @ts-expect-error با یک خط دلیل عوضش کن، تا وقتی خطا رفع شد خودش خبر بدهد")
    else:
        ok("STACK-05", f"در {files} فایلِ TypeScript سرکوبِ کورِ تایپ نیست")
...
PROBES = [
    ...
    ("STACK-05", "سرکوبِ کورِ تایپ نباشد (@ts-expect-error، نه سرکوبِ بی‌تاریخ)"),
]
