...
def main() -> int:
    ...
    # این بررسی فقط در مخزنِ منبع معنی دارد: کپی همه‌ی فایل‌های منبع را ندارد و
    # «ارجاعِ مرده» شمردنشان هشدارِ نادرست است. ولی **همیشه یک نتیجه ثبت می‌شود**،
    # وگرنه شمارنده در کپی یکی کم می‌آید و کفِ MIN_CHECKS بی‌خود شلیک می‌کند —
    # که دقیقاً روی اولین نصبِ واقعی اتفاق افتاد. قرمزیِ دروغ روی نصبِ تازه از
    # سبزِ توخالی بدتر است، چون یاد می‌دهد دروازه را نادیده بگیری (CORE-04).
    if is_origin:
        check(not dead, "هر مسیرِ فایلی که در مستندات آمده وجود دارد", " · ".join(sorted(dead)))
    else:
        check(True, "ارجاع‌های فایلی — فقط در مخزنِ منبع سنجیده می‌شود")
